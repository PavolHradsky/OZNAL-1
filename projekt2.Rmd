...
Podľa korelácie z EDA vybraté prediktory **GDP a percentage expenditure** súvisia s **rozvojovým statusom krajiny**: 
```{r}
data %$% pairs( status_oh ~  `percentage expenditure` + GDP)
```

Podľa pairplotu z EDA vybraté prediktory **GDP a Schooling** súvisia s **rozvojovým statusom krajiny**: 
```{r}
...
Accuracy je 86,24%. H0 zamietame. Hodnota kappa 0.5833 naznačuje strednú až dobrú zhodu medzi pozorovaniami a predpovedaniami modelu.
eps-regression - model sa snaží predpovedať spojitú premennú namiesto klasifikácie do dvoch tried. 
...
Random Forest ma problémy s chýbajúcimi hodnotammi v train_data, môžem ich omitnut
...
Model LDA má najvyššiu presnosť zo všetkých modelov (0,8624) spolu s SVM-radial.
...
# Model
rf_model <- randomForest(status_oh ~ log_GDP + Schooling, data = train_data_rf)
rf_model
# Predictions
rf_predictions <- predict(rf_model, test_data)
...
Accuracy je 79,91%. H0 zamietame.
...
Model Naive Bayes má najvyššiu presnosť zo všetkých modelov (0,851). Model LDA má najnižšiu presnosť (0,799) spolu s GLM.
Model SVM-linear má omnoho lepšiu presnosť ako v predošlom experimente (0,829).
Model Naive Bayes má najvyššiu citlivosť (0,874), čo naznačuje, že efektívne identifikuje pozitívne prípady. Modely Random Forrest, GLM, LDA majú najvyššiu špecificitu (0,948), čo naznačuje, že efektívne identifikujú negatívne prípady. 
...
Model SVM-radial má najvyššiu presnosť zo všetkých modelov (0,851). Model LDA má najnižšiu presnosť (0,731) spolu s GLM.
Model SVM-radial má najvyššiu citlivosť (0,897), čo naznačuje, že efektívne identifikuje pozitívne prípady. Model SVM-radial majú výrazne nižšiu špecificitu (0,628), čo naznačuje, že neefektívne identifikuje negatívne prípady.
...
- posnažíme sa zlepšiť hodnotu specificity pre SVm radial kernel
```{r}
...
tuning_grid <- expand.grid(sigma = seq(0.1, 1, by = 0.1), C = 2^(-2:2))

#model
model <- train(status_oh ~ GDP + `percentage expenditure`, data = data %>% drop_na(), 
               trControl = train_control, 
               method = "svmRadial",tuneGrid = tuning_grid)
...
actual <- test_data$status_oh
roc_svm_rad = rocit(svm_scores, actual)
cutoff_index <- which.max(roc_svm_rad$TPR + (1 - roc_svm_rad$FPR) - 1)
optimal_cutoff <- roc_svm_rad$Cutoff[cutoff_index]
predicted_class_svm_rad <- ifelse(svm_scores >= optimal_cutoff, 1, 0)
...
V prvom experimente, kde sme predikovali status_oh pomocou GDP a Schooling, nám ako najlepší model vyšiel SVM s radial kernelom s accuracy 86%. To je oproti SVM s linearnym kernelom zlepšenie o takmer 30%. Je to preto, že GDP a Schooling nie su lineárne závislé, čo sme mali možnosť vidieť aj na pairplote.

V druhom experimente sme vytvorili modely na rovnakých premenných, ale GDP sme zlogaritmovali, s cieľom zlepšiť výsledky pre SVM linear kernel. Tu mal najlepší výsledok Naive Bayes model, s accuracy 85%. SVM s radiálnym kernelom mal 84%, SVM s lineárnym kernelom mal až 82% accuracy, čo je omnoho viac ako v predošlom experimente. To znamená, že zlogaritmovanie premennej zmenilo exponenciálnu závislosť na lineárnu, a tým pádom sme zdvihli úspešnosť SVM s lineárnym kernelom.

V poslednom experimente sme znovu predikovali status_oh, tentokrát na základe GDP a percentage expenditure. Tu nám znovu vyšiel ako najlepší model SVM s radiálnym kernelom s accuracy 85%. Zaujímavé ale je, že specificity má omnoho nižšiu ako ostatné modely - 62%, pričom ostatné majú okolo 80%. To znamená, že ak nám záleží na minimalizovaní falošne negatívnych prípadov, mali by sme v tomto prípade, pri tejto kombinácii prediktorov, radšej zvoliť iný model ako SVM s radiálnym kernelom. Vyskúšali sme preto aj hyperparameter tuning pre tento model, no výsledok pre specificity sa nezlepšil.
