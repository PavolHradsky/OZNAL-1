...
ggplot(data, aes(x = `percentage expenditure`, y = GDP, color = factor(status_oh))) +
  geom_point() +
  labs(title = "Pairplot of 'percentage expenditure' and 'GDP' with different colors for 'status_oh'",
       x = "percentage expenditure", y = "GDP")
```
...
ggplot(data, aes(x = GDP, y = Schooling, color = factor(status_oh))) +
  geom_point() +
  labs(title = "GDP' and 'Schooling' with different colors for 'status_oh'",
       x = "GDP", y = "Schooling")
```
...
ggplot(data, aes(x = log(GDP), y = Schooling, color = factor(status_oh))) +
  geom_point() +
  labs(title = "log(GDP)' and 'Schooling' with different colors for 'status_oh'",
       x = "log(GDP)", y = "Schooling")
```
...
```{r}
#install.packages("randomForest") # uncomment if you don't have the package installed
...
Model Naive Bayes má najvyššiu špecificitu (0,9318), čo naznačuje, že efektívne identifikuje negatívne prípady, naopak SVM-linear malo najnižšiu a to výrazne.
...
Celkovo model naznačuje, že GDP a percentage expenditure sú významnými prediktormi rozvojového statusu krajiny, pričom vyššie GDP a percentage expenditure sú spojené s vyššou pravdepodobnosťou byť rozvinutou krajinou. H0 zamietame. Spravili sme cross tabulaciu a z confusion matrix vidíme, že náš model má 81,13% presnosť. 
...
percentage expendicture viac vplýva na status krajiny (rozvinutá/nerozvinutá) ako GDP z grafu pre logistickú funkciu, čo nám nakoniec hovorí aj samotný model.
...
- pairplot kombinácie percentage expenditure a Schooling
```{r}
data %$% pairs(status_oh ~  `percentage expenditure` + Schooling)
```

# Hyperparameter tuning pre SVM radial kernel, prediktory sú GDP a percentage expenditure
...
```{r}
#install.packages("kernlab") # uncomment if you don't have the package installed
set.seed(123)
# one-hot encoding column status
data$status_oh <- as.factor(data$status_oh)
# 10 násobná cv - cross validation, data sa rozdelia na 10 rovnomerných častí a proces sa opakuje 10 krát
train_control <- trainControl(method = "cv", 
                              number = 10)

tuning_grid <- expand.grid(C = 2^(-5:5))
...
Hodnota C = 4 bola vybraná ako optimálna, pretože poskytla najväčšiu presnosť.
...
Random Forrest má najvyššiu specificity (0,6363).
...
Random forrest bol stabilný a výkonný model. SVM s radiálnym kernelom bol tiež dobrý, dokázal sa lepšie adaptovať. Nárast sensitivity môže viesť k poklesu specificity. Zlogaritmovaním GDP sa zlepšujú výsledky SVM s lineárnym kernelom, to naznačuje, že lineárne modely môžu byť účinnejšie na zlogaritmovaných dátach, kedže sú lepšie normalizované.