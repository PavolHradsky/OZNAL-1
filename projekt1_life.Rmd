...
# ROC analysis
library(pROC)
roc_nb <- roc(test_data$status_oh, nb_predictions[, "1"])
plot(roc_nb)

# Calculate optimal cutoff
cutoff_index <- which.max(roc_nb$specificities + roc_nb$sensitivities - 1)
optimal_cutoff <- roc_nb$thresholds[cutoff_index]

# Predicted class based on optimal cutoff
predicted_class_nb <- ifelse(nb_predictions[, "1"] >= optimal_cutoff, 1, 0)
...
# give me number of missing values in each column
```{r}
...
# Model
rf_model <- randomForest(status_oh ~ GDP + Schooling, data = na.omit(train_data))
summary(rf_model)
# Predictions
...
# ROC analysis
roc_lda <- roc(test_data$status_oh, lda_predictions$posterior[, "1"])
plot(roc_lda)

# Calculate optimal cutoff
cutoff_index <- which.max(roc_lda$specificities + roc_lda$sensitivities - 1)
optimal_cutoff <- roc_lda$thresholds[cutoff_index]

# Predicted class based on optimal cutoff
predicted_class_lda <- ifelse(lda_predictions$posterior[, "1"] >= optimal_cutoff, 1, 0)
...
data.frame(
  Model = c("LDA", "SVM-linear","SVM-radial", "Naive Bayes", "Random Forest"),
  Accuracy = c(
...
# plot also ROC curves, younden index values into tables
data.frame(
  Model = c("LDA", "SVM-linear","SVM-radial", "Naive Bayes", "Random Forest"),
  Youden_Index = c(
    max(roc_lda$sensitivities + roc_lda$specificities - 1),
    max(roc_svm_ln$sensitivities + roc_svm_ln$specificities - 1),
    max(roc_svm_rad$sensitivities + roc_svm_rad$specificities - 1),
    max(roc_nb$sensitivities + roc_nb$specificities - 1),
    max(roc_random_forrest$sensitivities + roc_random_forrest$specificities - 1)
  )
)
...
# Model
svm_model <- svm(status_oh ~ `Life expectancy` + `Adult Mortality` + Alcohol  +
              `percentage expenditure` + BMI + 
              `Total expenditure` + Diphtheria + `HIV/AIDS` + GDP +
              `Income composition of resources` + Schooling, data = train_data, kernel = "radial")
summary(svm_model)
# Predictions
svm_predictions <- predict(svm_model, test_data)

actual <- test_data$status_oh
roc = rocit(svm_predictions, actual)
plot(roc)
cutoff_index <- which.max(roc$TPR + (1 - roc$FPR) - 1)
optimal_cutoff <- roc$Cutoff[cutoff_index]
optimal_cutoff

predicted_class <- ifelse(svm_predictions >= optimal_cutoff, 1, 0)

# Confusion matrix
confusionMatrix(as.factor(predicted_class), as.factor(test_data$status_oh))
...
# Model
#rf_model <- randomForest(status_oh ~ `Life expectancy` + `Adult Mortality` + Alcohol  +
#              `percentage expenditure` + BMI + 
#              `Total expenditure` + Diphtheria + `HIV/AIDS` + GDP +
...
#actual <- test_data$status_oh
#roc = rocit(rf_predictions, actual)
#plot(roc)
#cutoff_index <- which.max(roc$TPR + (1 - roc$FPR) - 1)
#optimal_cutoff <- roc$Cutoff[cutoff_index]
#optimal_cutoff

#predicted_class <- ifelse(rf_predictions >= optimal_cutoff, 1, 0)
...
# Model
nb_model <- naiveBayes(status_oh ~ `Life expectancy` + `Adult Mortality` + Alcohol  +
              `percentage expenditure` + BMI + 
              `Total expenditure` + Diphtheria + `HIV/AIDS` + GDP +
              `Income composition of resources` + Schooling, data = train_data)
summary(nb_model)
...
# ROC analysis
library(pROC)
roc <- roc(test_data$status_oh, nb_predictions[, "1"])
plot(roc)

# Calculate optimal cutoff
cutoff_index <- which.max(roc$specificities + roc$sensitivities - 1)
optimal_cutoff <- roc$thresholds[cutoff_index]

# Predicted class based on optimal cutoff
predicted_class <- ifelse(nb_predictions[, "1"] >= optimal_cutoff, 1, 0)
